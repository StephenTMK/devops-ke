...
# Regex keeps everything after the prefix in capture group 2, forwarded via rewrite-target "/$2".
resource "kubernetes_ingress_v1" "localstack_ingress" {
  ...
  spec {
    ...
    # Wildcard host rule (plain ngrok host)
    rule {
      http {
        path {
          path      = "/localstack(/|$)(.*)"
...
# (No configuration-snippet; snippets are disabled on your controller.)
